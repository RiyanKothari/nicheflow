...
import { useGetDashboardStats } from "@/hooks/data";
...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch("/api/orchestrator/weekly-digest", { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.ok ? r.json() : null)
  ...
  useEffect(() => {
    const token = localStorage.getItem("nf_token");
    if (!token) return;
    fetch("/api/orchestrator/next-action", { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.ok ? r.json() : null)
